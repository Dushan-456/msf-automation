  ...
  return (
    <div className="p-5 max-w-7xl mx-auto h-[calc(100vh)] flex flex-col pt-6">
      <div className="mb-4 shrink-0 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 border-b-4 border-emerald-500 pb-2 inline-block">
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 bg-white rounded-2xl border border-slate-100 shadow-sm py-16">
          ...
          <p className="mt-1 text-slate-400">There are currently no surveys in the analyzed folder.</p>
        </div>
      ) : (
          ...
          <div className="flex flex-col gap-4">
            {surveys.map((survey, idx) => (
              <div
                ...
                className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all flex items-center justify-between group relative overflow-hidden cursor-pointer"
              >
                ...
                <div className="flex items-center gap-4 flex-1 pr-6">
                  {/* Rank badge */}
                  <div className="bg-slate-100 group-hover:bg-emerald-50 w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors">
                    <span className="text-sm font-extrabold text-slate-500 group-hover:text-emerald-700 transition-colors">
                      {(page - 1) * perPage + idx + 1}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <h3
                      className="text-lg font-bold text-slate-800 leading-tight group-hover:text-emerald-700 transition-colors truncate max-w-lg lg:max-w-2xl"
                      title={survey.title}
                    >
                      {survey.title}
                    </h3>
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <div className="text-right">
                    ...
                    <div className="inline-flex items-center justify-center bg-emerald-50 text-emerald-700 font-bold px-3 py-1 rounded-full text-sm min-w-[3rem]">
