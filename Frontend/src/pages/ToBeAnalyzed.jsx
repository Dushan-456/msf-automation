...
export default function ToBeAnalyzed() {
  ...
  const fetchAnalyzedSurveys = async (pageNum) => {
    setLoading(true);
    setError('');
    try {
      const res = await axios.get(`${API_URL}/reports/analyzed`, {
      ...
      setError(
        isRateLimit
          ...
          : 'Failed to fetch analyzed surveys. Please try again later.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-5 max-w-7xl mx-auto h-[calc(100vh)] flex flex-col pt-6 bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      <div className="mb-4 shrink-0 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white border-b-4 border-emerald-500 pb-2 inline-block">
            To Be Analyzed Surveys
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            Surveys that have been analyzed, sorted by total responses.
          </p>
        </div>
        <button
          onClick={() => fetchAnalyzedSurveys(page)}
          disabled={loading}
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">Loading surveys...</p>
          </div>
        </div>
      ) : surveys.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 shadow-sm py-16">
          ...
          <p className="text-xl font-medium text-slate-600 dark:text-slate-200">No surveys found</p>
          <p className="mt-1 text-slate-400 dark:text-slate-500">There are currently no surveys in the analyzed folder.</p>
        </div>
      ) : (
          ...
          <div className="flex flex-col gap-4">
            {surveys.map((survey, idx) => (
              <div
                ...
                className="bg-white dark:bg-gray-900 rounded-xl p-5 border border-slate-100 dark:border-gray-800 shadow-sm hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-500/50 transition-all flex items-center justify-between group relative overflow-hidden cursor-pointer"
              >
                ...
                <div className="flex items-center gap-4 flex-1 pr-6">
                  {/* Rank badge */}
                  <div className="bg-slate-100 dark:bg-gray-800 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20 w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors font-bold">
                    <span className="text-sm font-extrabold text-slate-500 dark:text-slate-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                      {(page - 1) * perPage + idx + 1}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <h3
                      className="text-lg font-bold text-slate-800 dark:text-white leading-tight group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors truncate max-w-lg lg:max-w-2xl"
                      title={survey.title}
                    >
                      {survey.title}
                    </h3>
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <div className="text-right">
                    ...
                    <div className="inline-flex items-center justify-center bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 font-bold px-3 py-1 rounded-full text-sm min-w-[3rem]">
