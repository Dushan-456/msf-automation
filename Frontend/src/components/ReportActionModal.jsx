...
export default function ReportActionModal({ survey, onClose, onComplete }) {
  const [step, setStep] = useState('fetching'); // fetching, printing, ready, finalizing, done, error
            ...
            {step === 'ready' && (
              <>
                <div className="flex items-center justify-center w-16 h-16 rounded-full bg-blue-100 mb-2">
                  <svg className="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                </div>
                <p className="text-xl text-gray-900 font-bold">Report Ready</p>
                <p className="text-sm text-gray-500 text-center">Your PDF report is ready.<br/>Click below to download and complete the analysis.</p>
                <div className="mt-4 w-full px-4">
                  <button onClick={handleDownloadAndFinalize} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors shadow-sm">
                    Download & Complete
                  </button>
                </div>
              </>
